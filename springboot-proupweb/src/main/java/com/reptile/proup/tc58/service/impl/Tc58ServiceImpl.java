...
//				"合肥":"hf|837"
    ...
    public boolean updateQueryParams(String page) {
        ...
        try {
            ...
            //参数 class="item"
            Elements params_items = doc.getElementsByClass("item");

            //薪资参数节点
            Elements filterSalary = params_items.select("#filterSalary");

            Elements salaryATag = filterSalary.select("a");

            String _ix = "=";
            Integer xinzi = 0;
            //<a data-value="postdate=1000_1999" href="https://bj.58.com/job/?minxinzi=1000_1999" rel="nofollow">1000-2000元</a>
            for (Element aTag : salaryATag) {
                String datavalue = aTag.attr("data-value");
                if (datavalue != null && !datavalue.isEmpty()) {
                    if (datavalue.indexOf("_") != -1) {
                        String minxinzi = datavalue.substring(datavalue.indexOf("_"), datavalue.length());
                    }
